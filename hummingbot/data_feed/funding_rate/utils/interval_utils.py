...
class IntervalUtility:
    _INTERVAL_TO_MILLISECONDS: Dict[IntervalType, int] = {
        "1h": 60 * 60 * 1000,
        "4h": 4 * 60 * 60 * 1000,
        "8h": 8 * 60 * 60 * 1000,
    }
    ...
    @staticmethod
    def get_duration_ms(interval: IntervalType) -> int:
    ...
    @staticmethod
    def align_timestamp(timestamp_ms: int, interval: IntervalType) -> int:
        interval_ms: int = IntervalUtility.get_duration_ms(interval)
        return (timestamp_ms // interval_ms) * interval_ms

    @staticmethod
    def get_next_interval_timestamp(current_timestamp_ms: int, interval: IntervalType) -> int:
        interval_ms: int = IntervalUtility.get_duration_ms(interval)
        aligned_timestamp: int = IntervalUtility.align_timestamp(current_timestamp_ms, interval)
        return aligned_timestamp + interval_ms

    @staticmethod
    def get_previous_interval_timestamp(current_timestamp_ms: int, interval: IntervalType) -> int:
        interval_ms: int = IntervalUtility.get_duration_ms(interval)
        aligned_timestamp: int = IntervalUtility.align_timestamp(current_timestamp_ms, interval)
