...
class BybitPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def authenticator(self) -> BybitPerpetualAuth:
        return BybitPerpetualAuth(
            self.bybit_perpetual_api_key, self.bybit_perpetual_secret_key, self._time_synchronizer
        )
    ...
    async def _update_balances(self):
        ...
        unified_wallet_response = await self._api_get(
            path_url=CONSTANTS.GET_WALLET_BALANCE_PATH_URL, params={"accountType": "UNIFIED"}, is_auth_required=True
        )
        ...
        unified_wallet_balance = [
            d for d in unified_wallet_response["result"]["list"][0]["coin"] if Decimal(d["equity"]) > 0
        ]
        ...
        for asset in unified_wallet_balance:
            ...
            self._account_available_balances[asset["coin"]] = (
                Decimal(asset["walletBalance"])
                - Decimal(asset["locked"])
                - Decimal(asset["totalOrderIM"])
                - Decimal(asset["totalPositionMM"])
                - Decimal(asset["totalPositionIM"])
            )
    ...
    def _process_wallet_event_message(self, wallet_msg: Dict[str, Any]):
        ...
        self._account_available_balances[symbol] = (
            Decimal(wallet_msg["walletBalance"])
            - Decimal(wallet_msg["locked"])
            - Decimal(wallet_msg["totalOrderIM"])
            - Decimal(wallet_msg["totalPositionMM"])
            - Decimal(wallet_msg["totalPositionIM"])
        )
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        ...
        self.logger().info(f"There are {len(mapping)} trading pairs in the mapping")
